#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <stdio.h>
#include <signal.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
#include <sys/time.h>
#include <sys/types.h>
#include <sys/termios.h>
#include <sys/mman.h>

/* Registers */
enum
{
    R_R0 = 0,
    R_R1,
    R_R2,
    R_R3,
    R_R4,
    R_R5,
    R_R6,
    R_R7,
    R_PC,   /* program counter */
    R_COND, /* condition flags */
    R_COUNT
};

/* Condition Flags */
enum
{
    FL_POS = 1 << 0, /* P */
    FL_ZRO = 1 << 1, /* Z */
    FL_NEG = 1 << 2  /* N */
};

/* Opcodes */
enum
{
    OP_BR = 0, /* branch */
    OP_ADD,    /* add  */
    OP_LD,     /* load */
    OP_ST,     /* store */
    OP_JSR,    /* jump register */
    OP_AND,    /* bitwise and */
    OP_LDR,    /* load register */
    OP_STR,    /* store register */
    OP_RTI,    /* unused */
    OP_NOT,    /* bitwise not */
    OP_LDI,    /* load indirect */
    OP_STI,    /* store indirect */
    OP_JMP,    /* jump */
    OP_RES,    /* reserved (unused) */
    OP_LEA,    /* load effective address */
    OP_TRAP    /* execute trap */
};

/* Memory Mapped Registers */
enum
{
    MR_KBSR = 0xFE00, /* keyboard status */
    MR_KBDR = 0xFE02  /* keyboard data */
};

/* TRAP Codes */
enum
{
    TRAP_GETC = 0x20,  /* get character from keyboard */
    TRAP_OUT = 0x21,   /* output a character */
    TRAP_PUTS = 0x22,  /* output a word string */
    TRAP_IN = 0x23,    /* input a string */
    TRAP_PUTSP = 0x24, /* output a byte string */
    TRAP_HALT = 0x25   /* halt the program */
};

/* Memory Storage */
#define MEMORY_MAX (1 << 16)
uint16_t memory[MEMORY_MAX];

/* Register Storage */
uint16_t reg[R_COUNT];

/* Sign extend a 5-bit value to 16 bits */
uint16_t sign_extend(uint16_t x, int bit_count)
{
    if ((x >> (bit_count - 1)) & 1) {
        x |= (0xFFFF << bit_count);
    }
    return x;
}

/* Update the condition flags based on the current value in a register */
void update_flags(uint16_t r)
{
    if (reg[r] == 0) {
        reg[R_COND] = FL_ZRO;
    } else if (reg[r] >> 15) { /* a 1 in the left-most bit indicates negative */
        reg[R_COND] = FL_NEG;
    } else {
        reg[R_COND] = FL_POS;
    }
}

/* Function Prototypes */
int check_key();

/* Memory Access */
uint16_t mem_read(uint16_t address)
{
    if (address == MR_KBSR) {
        if (check_key()) {
            memory[MR_KBSR] = (1 << 15);
            memory[MR_KBDR] = getchar();
        } else {
            memory[MR_KBSR] = 0;
        }
    }
    return memory[address];
}

void mem_write(uint16_t address, uint16_t val)
{
    memory[address] = val;
}

/* Check if a key is pressed */
int check_key()
{
    fd_set readfds;
    FD_ZERO(&readfds);
    FD_SET(STDIN_FILENO, &readfds);

    struct timeval timeout;
    timeout.tv_sec = 0;
    timeout.tv_usec = 0;
    return select(1, &readfds, NULL, NULL, &timeout) != 0;
}

/* Read an image file into memory */
void read_image_file(FILE* file)
{
    /* the origin tells us where in memory to place the image */
    uint16_t origin;
    fread(&origin, sizeof(origin), 1, file);
    origin = (origin << 8) | (origin >> 8);

    /* we know the maximum file size so we only need one fread */
    uint16_t max_read = MEMORY_MAX - origin;
    uint16_t* p = memory + origin;
    size_t read = fread(p, sizeof(uint16_t), max_read, file);

    /* swap to little endian */
    while (read-- > 0) {
        *p = (*p << 8) | (*p >> 8);
        ++p;
    }
}

/* Set the PC to starting position */
int read_image(const char* image_path)
{
    FILE* file = fopen(image_path, "rb");
    if (!file) { return 0; }
    read_image_file(file);
    fclose(file);
    return 1;
}

/* Main execution loop */
int main(int argc, const char* argv[])
{
    /* Load arguments */
    if (argc < 2) {
        /* show usage string */
        printf("lc3 [image-file1] ...\n");
        exit(2);
    }

    for (int j = 1; j < argc; ++j) {
        if (!read_image(argv[j])) {
            printf("failed to load image: %s\n", argv[j]);
            exit(1);
        }
    }

    /* Setup */
    enum { PC_START = 0x3000 }; /* Start program at x3000 */
    reg[R_PC] = PC_START;
    int running = 1;

    while (running) {
        /* FETCH */
        uint16_t instr = mem_read(reg[R_PC]++);
        uint16_t op = instr >> 12;

        switch (op) {
            case OP_ADD: {
                /* ADD: Add two values */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register (DR) */
                uint16_t r1 = (instr >> 6) & 0x7;  /* first operand (SR1) */
                uint16_t imm_flag = (instr >> 5) & 0x1;  /* immediate mode flag */

                if (imm_flag) {
                    uint16_t imm5 = sign_extend(instr & 0x1F, 5);
                    reg[r0] = reg[r1] + imm5;
                } else {
                    uint16_t r2 = instr & 0x7;  /* second operand (SR2) */
                    reg[r0] = reg[r1] + reg[r2];
                }
                update_flags(r0);
                break;
            }
            case OP_AND: {
                /* AND: Bitwise AND */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register (DR) */
                uint16_t r1 = (instr >> 6) & 0x7;  /* first operand (SR1) */
                uint16_t imm_flag = (instr >> 5) & 0x1;  /* immediate mode flag */

                if (imm_flag) {
                    uint16_t imm5 = sign_extend(instr & 0x1F, 5);
                    reg[r0] = reg[r1] & imm5;
                } else {
                    uint16_t r2 = instr & 0x7;  /* second operand (SR2) */
                    reg[r0] = reg[r1] & reg[r2];
                }
                update_flags(r0);
                break;
            }
            case OP_NOT: {
                /* NOT: Bitwise complement */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register (DR) */
                uint16_t r1 = (instr >> 6) & 0x7;  /* source register (SR) */
                reg[r0] = ~reg[r1];
                update_flags(r0);
                break;
            }
            case OP_BR: {
                /* BR: Conditional branch */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                uint16_t cond_flag = (instr >> 9) & 0x7;  /* condition flags */
                if (cond_flag & reg[R_COND]) {
                    reg[R_PC] += pc_offset;
                }
                break;
            }
            case OP_JMP: {
                /* JMP: Jump to address in register */
                uint16_t r1 = (instr >> 6) & 0x7;  /* base register */
                reg[R_PC] = reg[r1];
                break;
            }
            case OP_JSR: {
                /* JSR: Jump to subroutine */
                uint16_t long_flag = (instr >> 11) & 0x1;
                reg[R_R7] = reg[R_PC];
                
                if (long_flag) {
                    /* JSR */
                    uint16_t long_pc_offset = sign_extend(instr & 0x7FF, 11);
                    reg[R_PC] += long_pc_offset;
                } else {
                    /* JSRR */
                    uint16_t r1 = (instr >> 6) & 0x7;
                    reg[R_PC] = reg[r1];
                }
                break;
            }
            case OP_LD: {
                /* LD: Load PC-relative */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                reg[r0] = mem_read(reg[R_PC] + pc_offset);
                update_flags(r0);
                break;
            }
            case OP_LDI: {
                /* LDI: Load indirect */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                reg[r0] = mem_read(mem_read(reg[R_PC] + pc_offset));
                update_flags(r0);
                break;
            }
            case OP_LDR: {
                /* LDR: Load base+offset */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register */
                uint16_t r1 = (instr >> 6) & 0x7;  /* base register */
                uint16_t offset = sign_extend(instr & 0x3F, 6);  /* 6-bit offset */
                reg[r0] = mem_read(reg[r1] + offset);
                update_flags(r0);
                break;
            }
            case OP_LEA: {
                /* LEA: Load effective address */
                uint16_t r0 = (instr >> 9) & 0x7;  /* destination register */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                reg[r0] = reg[R_PC] + pc_offset;
                update_flags(r0);
                break;
            }
            case OP_ST: {
                /* ST: Store register (PC-relative) */
                uint16_t r0 = (instr >> 9) & 0x7;  /* source register */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                mem_write(reg[R_PC] + pc_offset, reg[r0]);
                break;
            }
            case OP_STI: {
                /* STI: Store indirect */
                uint16_t r0 = (instr >> 9) & 0x7;  /* source register */
                uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
                mem_write(mem_read(reg[R_PC] + pc_offset), reg[r0]);
                break;
            }
            case OP_STR: {
                /* STR: Store base+offset */
                uint16_t r0 = (instr >> 9) & 0x7;  /* source register */
                uint16_t r1 = (instr >> 6) & 0x7;  /* base register */
                uint16_t offset = sign_extend(instr & 0x3F, 6);  /* 6-bit offset */
                mem_write(reg[r1] + offset, reg[r0]);
                break;
            }
            case OP_TRAP: {
                /* TRAP: System call */
                reg[R_R7] = reg[R_PC];  /* save PC */
                
                switch (instr & 0xFF) {
                    case TRAP_GETC:  /* Get character from keyboard */
                        reg[R_R0] = (uint16_t)getchar();
                        update_flags(R_R0);
                        break;
                        
                    case TRAP_OUT:  /* Output a character */
                        putc((char)reg[R_R0], stdout);
                        fflush(stdout);
                        break;
                        
                    case TRAP_PUTS: {  /* Output a string */
                        uint16_t* c = memory + reg[R_R0];
                        while (*c) {
                            putc((char)*c, stdout);
                            c++;
                        }
                        fflush(stdout);
                        break;
                    }
                    
                    case TRAP_IN: {  /* Get string from keyboard */
                        printf("Enter a character: ");
                        char c = getchar();
                        putc(c, stdout);
                        reg[R_R0] = (uint16_t)c;
                        update_flags(R_R0);
                        break;
                    }
                    
                    case TRAP_PUTSP: {  /* Output a byte string */
                        uint16_t* c = memory + reg[R_R0];
                        while (*c) {
                            char char1 = (*c) & 0xFF;
                            putc(char1, stdout);
                            char char2 = (*c) >> 8;
                            if (char2) putc(char2, stdout);
                            c++;
                        }
                        fflush(stdout);
                        break;
                    }
                    
                    case TRAP_HALT:  /* Halt execution */
                        puts("HALT");
                        fflush(stdout);
                        running = 0;
                        break;
                }
                break;
            }
            case OP_RES:
            case OP_RTI:
            default:
                /* Bad opcode */
                printf("Bad opcode: 0x%04X\n", op);
                running = 0;
                break;
        }
    }

    return 0;
}
